...
let rec big_step t =
  match t with
    ...
    |TmPred(t1) when (big_step t1 = TmZero) ->
      big_step t1
    |TmPred(t1) when (is_a_num_value (big_step t1)) ->
      let TmSucc(t2) = big_step t1 in big_step(t2)
    |TmIsZero(t1) when (big_step t1 = TmZero) ->
      TmTrue
    |TmIsZero(TmSucc(t1)) when (is_a_num_value (big_step t1)) ->
      TmFalse
    |_ -> raise NO_RULE;;
...
let rec ne_big_step t =
    ...
    |TmPred(t1) when (ne_big_step t1 = TmZero) ->
      (ne_big_step t1)
    |TmPred(t1) when (is_a_num_value (ne_big_step t1)) ->
      let TmSucc(t2) = ne_big_step t1 in ne_big_step(t2)
    |TmIsZero(t1) when (ne_big_step t1 = TmZero) ->
      (TmTrue)
    |TmIsZero(TmSucc(t1)) when (is_a_num_value (ne_big_step t1)) ->
      (TmFalse)
...
print_val TmZero;;
print_val TmTrue;;
print_val (ne_big_eval_parse "pred pred succ succ 0");;
print_val (ne_big_eval_parse "succ pred pred succ succ succ pred 0");;
print_val (ne_big_eval_parse "if iszero succ pred 0 then true else succ 0");;
print_string ("should be true: ");;
print_val (ne_big_eval_parse "if iszero pred 0 then (if true then (if false then 0 else true) else succ 0) else succ 0");;
print_val (big_eval_parse "pred pred succ succ 0");;
print_val (big_eval_parse "succ pred pred succ succ succ pred 0");;
print_val (big_eval_parse "if iszero succ pred 0 then true else succ 0");;
print_string ("should be true: ");;
print_val (big_eval_parse "if iszero pred 0 then (if true then (if false then 0 else true) else succ 0) else succ 0");;
print_val (eval_parse "pred pred pred succ succ 0");;
print_val (eval_parse "succ pred pred succ succ succ pred 0");;
print_val (eval_parse "if iszero succ pred 0 then true else succ 0");;
print_string ("should be true: ");;
print_val (eval_parse "if iszero pred 0 then (if true then (if false then 0 else true) else succ 0) else succ 0");;

print_string "\nEx. ppss(0)";;
if ne_big_eval_parse "pred pred succ succ 0" == TmZero
