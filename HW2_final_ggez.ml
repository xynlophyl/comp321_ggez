...
(* converts a string to a list of chars: 
  ...
  *)
  ...
  let eval_tester s1 s2 =
    ...
    print_string "ne_big_eval_parse: "; print_val (ne_big_eval_parse s1));;

  eval_tester "pred pred succ succ 0" "0";;
  eval_tester "succ pred pred succ succ succ pred 0" "succ succ 0";;
  eval_tester "if iszero succ pred 0 then true else false" "false";;
  eval_tester "if iszero pred 0 then (if true then (if false then 0 else true) else succ 0) else succ 0" "true";;
  eval_tester "if (iszero (succ 0)) then succ(succ(0)) else succ(0)" "succ 0";;
  eval_tester "succ(if (false) then succ(succ(0)) else succ(pred(succ(0))))" "succ succ 0";;
  eval_tester "iszero succ(succ(succ(0)))" "false";;
  eval_tester "if (if (false) then true else false) then true else (succ(0))" "succ 0";;

  print_string "\n TESTING FOR EXCEPTIONS IN ne_big_eval_parse \n";;
  print_val (pp "if");;
